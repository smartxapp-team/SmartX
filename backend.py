...
def fetch_attendance(username, session_cookies):
    ...
    try:
        soup = BeautifulSoup(response.text, 'lxml')
        table = soup.find_all('table', class_='table-head-fixed')[1]
        rows = table.tbody.find_all('tr')
        courses = []
        total_conducted = 0; total_attended = 0
        for row in rows:
            cells = [cell.get_text(strip=True) for cell in row.find_all('td')]
            if len(cells) > 8:
                try:
                    percentage = float(cells[7])
                    course_data = {"name": cells[2], "conducted": int(cells[5]), "attended": int(cells[6]), "percentage": percentage, "status": cells[8], "color_code": get_attendance_color(percentage)}
        ...
        attendance_data = {"courses": courses, "overall_percentage": round(overall_percentage, 2)}
        set_data_in_cache(username, 'att', attendance_data)
        return attendance_data
    except Exception as e: return {"error": f"Failed to parse attendance HTML: {e}"}
...
def fetch_bio_log_data(username, session_cookies):
    ...
    try:
        ...
        if not table or not table.tbody: return {"error": "Could not find biometric data table."}
        bio_log = []
        for row in table.tbody.find_all('tr'):
            cells = [cell.get_text(strip=True) for cell in row.find_all('td')]
            if len(cells) >= 7 and cells[1] and cells[3]:
                bio_log.append({"s_no": cells[0], "date": cells[3], "status": cells[6]})
        bio_data = {"bio_log": bio_log}
        set_data_in_cache(username, 'bio_log', bio_data)
        return bio_data
    except Exception as e: return {"error": f"Failed to parse biometric log HTML: {e}"}
